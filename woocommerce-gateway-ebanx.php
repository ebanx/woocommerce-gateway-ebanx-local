...
if ( ! class_exists( 'WC_EBANX' ) ) {
	...
	class WC_EBANX {
		...
		public function enqueue_all_pages_scripts() {
			wp_enqueue_script( 'ebanx_libjs', '//js.ebanxpay.com/ebanx-pay-libjs-latest.min.js', array(), WC_EBANX::get_plugin_version(), true );
		}
		...
		public function ebanx_admin_order_details( $order ) {
			...
			if ( $payment_hash ) {
				...
				$customer_document     = get_user_meta( $order->get_customer_id(), '_ebanx_document', true );
		...
		/**
		 * Get instalments select to new order
		 *
		 * @param WC_Order $order   Order data.
		 */
		public function get_instalments_admin_html( $order ) {
			// Only for new orders
