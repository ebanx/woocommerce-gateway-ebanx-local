...
export WP_ROOT=/usr/src/wordpress
...
#  wp core install --path=$WP_ROOT --url=$WOOCOMMERCE_URL --title="$EBANX_SITE_TITLE" --admin_user=$EBANX_ADMIN_USERNAME --admin_password=$EBANX_ADMIN_PASSWORD --admin_email=$EBANX_SITE_EMAIL --skip-email --allow-root
  wp config set WP_DEBUG true --raw --allow-root --path=$WP_ROOT
  wp config set WP_DEBUG_LOG true --raw --allow-root --path=$WP_ROOT
  wp config set WP_DEBUG_DISPLAY true --raw --allow-root --path=$WP_ROOT
